...
namespace EvolutionPC.Store
{
    ...
    public class SlotStoreController : MonoBehaviour, IAutoDestruct
    {
        ...
        public void BuyComponent()
        {
            ...
            if(TemporariStorageData.TakeMoney() >= Price)
            {

                TemporariStorageData.RemoveMoney(Price);

                TemporariStorageData.AddIntToPartDataStorage(Level, GameController.TellPartType());
